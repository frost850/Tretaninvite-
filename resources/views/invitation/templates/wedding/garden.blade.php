...
<link href="https://fonts.bunny.net/css?family=libre-baskerville:400,400i,700|nunito:300,400,600|josefin-sans:300,400,600&display=swap" rel="stylesheet" />
@endpush
...
<section class="hero" id="top">
    ...
    <div style="margin-bottom:48px; text-align:center;">
        ...
        <p class="fi" style="font-size:clamp(1.4rem,4vw,2rem); color:var(--fg); margin-top:6px;">{{ $guest->guest_name }}</p>
    </div>
    @endif

    <div class="hero-badge rv">?? Undangan Pernikahan ??</div>
    ...
    @endif
    @if($wedding->location)
    <p class="rv fj" style="margin-top:12px; font-size:.68rem; letter-spacing:.22em; color:var(--mut); text-transform:uppercase;">{{ $wedding->location }}</p>
    @endif
...
<section class="sec" id="mempelai">
    ...
    <div class="mp-grid">
        <div class="rv">
            ...
            <p class="mp-par">Putri dari<br><strong style="color:var(--fg);">{{ $wedding->bride_father ?? '' }}{{ ($wedding->bride_father && $wedding->bride_mother) ? ' & ' : '' }}{{ $wedding->bride_mother ?? '' }}</strong></p>
            @endif
        </div>
        ...
        <div class="rv">
            ...
            <p class="mp-par">Putra dari<br><strong style="color:var(--fg);">{{ $wedding->groom_father ?? '' }}{{ ($wedding->groom_father && $wedding->groom_mother) ? ' & ' : '' }}{{ $wedding->groom_mother ?? '' }}</strong></p>
            @endif
        </div>
    </div>
</section>
...
<section class="sec" id="acara">
    ...
    <div class="card-grid">
        ...
        <div class="card rv">
            ...
            <div class="cinfo">
                <strong style="color:var(--fg);">{{ ($wedding->akad_date ?? $wedding->event_date)->locale('id')->translatedFormat('l, d F Y') }}</strong><br>
        ...
        <div class="card rv">
            ...
            <div class="cinfo">
                <strong style="color:var(--fg);">{{ $wedding->event_date->locale('id')->translatedFormat('l, d F Y') }}</strong><br>
...
{{-- -- COUNTDOWN -- --}}
<section class="sec" style="background:var(--mint)80; max-width:100%; padding-left:0; padding-right:0;" id="countdown">
    <div style="max-width:680px; margin:0 auto; padding:80px 24px; text-align:center;">
        ...
        <div class="cdg rv">
            <div class="cdi"><span class="cdn" id="g-hari">00</span><span class="cdl">Hari</span></div>
            <span class="cds">�</span>
            <div class="cdi"><span class="cdn" id="g-jam">00</span><span class="cdl">Jam</span></div>
            <span class="cds">�</span>
            <div class="cdi"><span class="cdn" id="g-menit">00</span><span class="cdl">Menit</span></div>
            <span class="cds">�</span>
            <div class="cdi"><span class="cdn" id="g-detik">00</span><span class="cdl">Detik</span></div>
        </div>
    </div>
</section>
...
<section class="sec" id="galeri">
    ...
    @include('invitation.partials.gallery-innovations', [
        ...
        'galleryMode'    => 'slider',
        'galleryBgColor' => '#f0f7ed',
        'galleryAccent'  => 'rgba(56,132,65,0.25)',
    ])
</section>
@endif
...
<section class="sec" id="lokasi">
    ...
    <div class="map-fr rv">
        ...
        <div class="map-ph">
            ...
            <span class="fj" style="font-size:.68rem; letter-spacing:.22em; text-transform:uppercase; color:var(--mut);">{{ $wedding->location ?? 'Lokasi Acara' }}</span>
        </div>
        @endif
    </div>
...
<section id="amplop" style="background:var(--sage); padding:80px 24px;">
    <div style="max-width:680px; margin:0 auto; text-align:center;">
        ...
        <div class="gift-grid" style="margin-top:32px;">
            ...
            <div class="card rv">
                ...
                <button class="copy-btn" onclick="gdCopy('{{ $acc->account_number }}',this)">Salin Nomor</button>
            </div>
            @endforeach
            @else
            ...
            <div class="card rv">
                ...
                <button class="copy-btn" onclick="gdCopy('{{ $wedding->bride_norek }}',this)">Salin Nomor</button>
            </div>
            @endif
            ...
            <div class="card rv">
                ...
                <button class="copy-btn" onclick="gdCopy('{{ $wedding->groom_norek }}',this)">Salin Nomor</button>
            </div>
            @endif
            @endif
        </div>
...
<section class="rsvp-sec" id="rsvp">
    <div class="rsvp-inner">
        <div style="text-align:center;">
            <div class="leaf-row" style="color:var(--grn-l); margin-bottom:12px;">?? ?? ??</div>
            <span class="stag rv" style="color:var(--grn-l);">Konfirmasi &amp; Ucapan</span>
            <h2 class="stit rv" style="color:var(--mint);">Kirimkan Doa Anda</h2>
            <p class="ssub rv" style="color:rgba(209,250,229,.6);">Mohon konfirmasi kehadiran dan titipkan doa untuk kedua mempelai.</p>
        </div>

        <div class="rsvp-success" id="g-rsvp-ok">
            <div style="font-size:3rem; margin-bottom:16px;">??</div>
            <p class="fi" style="font-size:1.8rem; color:var(--grn-l);">Terima Kasih</p>
            <p style="color:rgba(209,250,229,.65); margin-top:8px; font-size:.88rem;">Konfirmasi dan ucapan Anda telah kami terima. Sampai jumpa! ??</p>
        </div>

        <form id="g-rsvp-form" class="rsvp-form rv" action="{{ route('rsvp.store', $wedding->slug) }}" method="POST">
            @csrf
            <div>
                <label for="g-name">Nama Lengkap</label>
                <input type="text" id="g-name" name="guest_name" value="{{ $guest->guest_name ?? '' }}" placeholder="Nama Anda" required>
            </div>
            <div>
                <label for="g-phone">Nomor WhatsApp <span style="text-transform:none;letter-spacing:0;opacity:.4;font-family:'Nunito',sans-serif;">(opsional)</span></label>
                <input type="tel" id="g-phone" name="phone" placeholder="+62 8xx xxxx xxxx">
            </div>
            <div>
                <label>Konfirmasi Kehadiran</label>
                <div class="hadir-toggle">
                    <input type="radio" name="attendance" id="g-hadir" value="hadir" checked>
                    <label for="g-hadir">Hadir</label>
                    <input type="radio" name="attendance" id="g-mungkin" value="mungkin">
                    <label for="g-mungkin">Mungkin</label>
                    <input type="radio" name="attendance" id="g-tidak" value="tidak_hadir">
                    <label for="g-tidak">Tidak Hadir</label>
                </div>
            </div>
            <div id="g-row-jml">
                <label for="g-jml">Jumlah Tamu</label>
                <input type="number" id="g-jml" name="guests_count" min="1" max="10" value="1">
            </div>
            <div>
                <label for="g-msg">Ucapan &amp; Doa</label>
                <textarea id="g-msg" name="message" placeholder="Tuliskan ucapan dan doa untuk kedua mempelai..."></textarea>
            </div>
        ...
        <div class="ucapan-list" style="margin-top:56px;">
            <div class="leaf-row" style="font-size:1.2rem; margin-bottom:24px; color:var(--grn-l);">?? ?? ??</div>
            <span class="stag" style="text-align:center; display:block; margin-bottom:24px; color:var(--grn-l);">Ucapan &amp; Doa</span>
...
{{-- -- FOOTER -- --}}
<footer class="footer">
    <div class="leaf-row" style="font-size:1.8rem; margin-bottom:20px;">?? ?? ??</div>
    ...
    <p class="footer-note">
        ...
        <em style="color:var(--grn); font-size:.9rem;">Dengan penuh cinta &amp; kebahagiaan ??</em>
    </p>
    <div class="g-div" style="margin-top:32px;"><span class="g-line"></span><span style="color:var(--leaf);">??</span><span class="g-line"></span></div>
