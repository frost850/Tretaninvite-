...
@section('title', 'Tambah Tamu — ' . $wedding->slug)
...
@section('content')
    ...
    <h1 class="text-2xl font-semibold text-slate-100 mb-2">Tambah Tamu</h1>
    ...
    <form action="{{ route('admin.guests.store') }}" method="post" class="space-y-4">
        ...
        <div class="adm-card p-6 space-y-4">
            <div>
                ...
                <input type="text" name="guest_name" id="guest_name" value="{{ old('guest_name') }}" required maxlength="255"
            ...
            <div>
                ...
                <input type="text" name="group_name" id="group_name" value="{{ old('group_name') }}" maxlength="100"
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="2" maxlength="500"
                          class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-slate-200 focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 focus:outline-none transition">{{ old('notes') }}</textarea>
            </div>
        </div>
