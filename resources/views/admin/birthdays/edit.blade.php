...
@section('title', 'Edit Undangan Birthday — ' . $w->bride_name)
...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            <div class="flex flex-wrap items-center gap-2 mb-1">
                <p class="text-slate-400 text-sm">🎂 Template: <strong class="text-pink-300">{{ $templateInfo['label'] }}</strong></p>
            ...
            <h1 class="text-2xl font-semibold text-slate-100">Edit Undangan Birthday</h1>
            <p class="text-slate-400 text-sm mt-1">{{ $w->bride_name }}
        ...
        <div class="flex gap-2">
            <a href="{{ route('invitation.show', $w->slug) }}" target="_blank"
               class="px-3 py-1.5 rounded-lg border border-white/10 text-slate-300 text-sm hover:bg-white/5 hover:text-pink-300 transition">
                👁 Preview
            </a>
    ...
    <form action="{{ route('admin.birthdays.update', $w->id) }}" method="post" class="space-y-0">
        @csrf
        @method('PUT')
        @include('admin.birthdays._form', ['w' => $w, 'template' => $w->template, 'templateInfo' => $templateInfo])

        <div class="flex flex-wrap gap-3 pt-2">
            <button type="submit"
                    class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-700 hover:to-rose-700 text-white font-bold shadow-lg transition-all">
                💾 Simpan Perubahan
            </button>
            <a href="{{ route('admin.weddings.index') }}"
    ...
    <div class="mt-8 p-5 rounded-xl border border-red-500/20 bg-red-900/10">
        ...
        <form action="{{ route('admin.birthdays.destroy', $w->id) }}" method="post"
              data-adm-confirm="Yakin hapus undangan birthday {{ addslashes($w->bride_name) }}? Semua data tamu ikut terhapus."
              data-adm-danger>
            @csrf
            @method('DELETE')
            <button type="submit"
