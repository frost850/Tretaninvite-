...
@section('title', 'Buat Undangan Anniversary — ' . $templateInfo['label'])
...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-slate-100">Buat Undangan Anniversary</h1>
        </div>
        <a href="{{ route('admin.weddings.create') }}" class="text-rose-400 hover:text-rose-300 hover:underline text-sm">← Ganti template</a>
    </div>
    ...
    <form action="{{ route('admin.anniversaries.store') }}" method="post">
        @csrf
        @if(!empty($orderId))
            <input type="hidden" name="order_id" value="{{ $orderId }}">
        @endif
        @include('admin.anniversaries._form', ['w' => null, 'template' => $template, 'templateInfo' => $templateInfo, 'package' => $package])
