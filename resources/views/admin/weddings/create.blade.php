...
@section('title', 'Buat Undangan — ' . $templateInfo['label'])
...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-slate-100">Buat Undangan Baru</h1>
        </div>
        <a href="{{ route('admin.weddings.create') }}" class="text-amber-400 hover:text-amber-300 hover:underline text-sm">← Ganti template</a>
    </div>
    ...
    <form action="{{ route('admin.weddings.store') }}" method="post">
        ...
        @include('admin.weddings._form', ['w' => null, 'template' => $template, 'templateInfo' => $templateInfo, 'package' => $package])
