document.addEventListener('DOMContentLoaded', function () {
    ...
    function tick() {
        const d = ed - new Date();
        if (d <= 0) { ['f-hari','f-jam','f-menit','f-detik'].forEach(id => document.getElementById(id) && (document.getElementById(id).textContent = '00')); return; }
        document.getElementById('f-hari').textContent  = String(Math.floor(d/864e5)).padStart(2,'0');
        document.getElementById('f-jam').textContent   = String(Math.floor(d%864e5/36e5)).padStart(2,'0');
        document.getElementById('f-menit').textContent = String(Math.floor(d%36e5/6e4)).padStart(2,'0');
        document.getElementById('f-detik').textContent = String(Math.floor(d%6e4/1e3)).padStart(2,'0');
    }
    ...
    document.querySelectorAll('input[name=attendance]').forEach(r => {
        r.addEventListener('change', () => {
            const row = document.getElementById('f-row-jml');
            if (row) row.style.display = r.value === 'tidak_hadir' ? 'none' : '';
        });
    });

    // RSVP submit
    const form = document.getElementById('f-rsvp-form');
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    document.getElementById('f-rsvp-ok').style.display = 'block';
