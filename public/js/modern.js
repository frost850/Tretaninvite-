document.addEventListener('DOMContentLoaded', function () {
    ...
    function tick() {
        const d = ed - new Date();
        if (d <= 0) { ['m-hari','m-jam','m-menit','m-detik'].forEach(id => document.getElementById(id) && (document.getElementById(id).textContent = '00')); return; }
        document.getElementById('m-hari').textContent  = String(Math.floor(d/864e5)).padStart(2,'0');
        document.getElementById('m-jam').textContent   = String(Math.floor(d%864e5/36e5)).padStart(2,'0');
        document.getElementById('m-menit').textContent = String(Math.floor(d%36e5/6e4)).padStart(2,'0');
        document.getElementById('m-detik').textContent = String(Math.floor(d%6e4/1e3)).padStart(2,'0');
    }
    ...
    document.querySelectorAll('input[name=attendance]').forEach(r => {
        r.addEventListener('change', () => {
            const row = document.getElementById('m-row-jml');
            if (row) row.style.display = r.value === 'tidak_hadir' ? 'none' : '';
        });
    });

    // RSVP submit
    const form = document.getElementById('m-rsvp-form');
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    document.getElementById('m-rsvp-ok').style.display = 'block';
